...
/* 演習6-14
 * 月を1～12の数値として表示して、その英語表記を入力させる英単語学習プログラムを作成せよ
 * ・出題する月の値は乱数で生成する。
 * ・学習者が望む限り、何度も繰り返せる
 * ・同一月を連続して出題しない。
 */
...
public class Ensyu6_14 {
	...
	public static void main(String[] args) {
		...
		int answerFlag = 1;
		// 前回と同じ問題を出さないため、前回の何月だったかを入れる判定用の変数
		int lastMonth = 0;
		// 出題する問題の月を入れる変数
		int randIndex = 0;
		...
		do {
			...
			if (answerFlag == 1) {
				...
				do {
					// 何月の月を入力させるか表示するため、ランダムな値を生成する
					randIndex = rand.nextInt(monthString.length);
				// 生成された問題が前回と同じ場合は再度乱数を生成する
				} while (lastMonth == randIndex);
			}

			// 問題に解答してもらうため、何月の英語を入力するか表示する
			System.out.print(randIndex + 1 + "月:");
			// 入力値をが正解か判定するために、変数に代入する
			String monthAnswer = standardInput.next();
			// 正解か判定するために入力値と配列で用意した月の英語名を比較する
			if (monthAnswer.equals(monthString[randIndex])) {
				// 比較の結果を表示する
